SMODS.Joker{ --Aikoyoris Bluetooth
    ...
    loc_txt = {
        ...
        ['text'] = {
            [1] = '{C:attention}#2#/#3#{} chance to create a',
            [2] = '{C:edition}Negative{} copy of the Joker',
    ...
    loc_vars = function(self, info_queue, card)
        local new_numerator, new_denominator = SMODS.get_probability_vars(card, 1, card.ability.extra.odds, 'j_modprefix_aikoyorisbluetooth') 
    ...
    calculate = function(self, card, context)
        if context.cardarea == G.jokers and context.joker_main  then
            if true then
                if SMODS.pseudorandom_probability(card, 'group_0_9195a6e3', 1, card.ability.extra.odds, 'j_modprefix_aikoyorisbluetooth', false) then
              local my_pos = nil
